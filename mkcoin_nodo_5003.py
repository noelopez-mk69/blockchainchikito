...
def mine_block():
    ...
    blockchain.add_transaction(sender = node_address, receiver = 'Mk', amount = 1)
    block = blockchain.create_block(proof, previous_hash)
    response = {'message': 'Aperro, Minaste un bloque!',
                'index': block['index'],
...
def is_valid():
    ...
    if is_valid:
        response = {'message':'Todo chido. El BLockchain es valido'}
    else:
        response = {'message':'Huy krnal lastima, el blockchain no es valido!'}
    return jsonify(response), 200
...
#Descentralizanding el Blockchain
...
def connect_node():
    ...
    response = {'message':'Todos los nodos estan ahora conectados. El MkCoin Blockchain contiene los siguientes nodos:',
